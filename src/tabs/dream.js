...
// LLM(JSON) 견고 파서 — services/llm-json-parser.js 로 추출(순수 로직, DOM 무의존).
// dream.js 는 내부 사용 + 공개 표면 재노출(권위 분리 금지). 산식/throw 동작 무변경.
// 안전망 = tests/test_llm_json_parser_runtime.py.
import { parseLLMJson, _sliceBalancedJson } from '../services/llm-json-parser.js';
export { parseLLMJson, _sliceBalancedJson } from '../services/llm-json-parser.js';
...
// 꿈 로또(재미용) 순수 로직은 services/dream-lotto.js 로 추출(2026-06-16, 동작보존).
// renderLotto/rerollLotto 의 DOM·이벤트 의존부만 이 파일에 남는다(로컬 import 로 호출).
import { generateLottoNumbers, ballRange } from '../services/dream-lotto.js';
export { LOTTO_FREQ, SYMBOL_NUMBERS, getEnergyWeights, dreamHash, seededRandom, weightedPick, ballRange, generateLottoNumbers } from '../services/dream-lotto.js';
