...
## §11 신규 카테고리 실 작동 확증 (2026-05-21 후속)

preview 재시작 (b5h2gg1ex) + cache clear + dev_unlock=premium 후 다중 입력:

| 입력 | 매칭 카테고리 | premium 자동 unlock |
|------|--------------|---------------------|
| "엄마랑 시험보러 갔어" | 👨‍👩‍👧 가족 (가족 > 시험 specific 순서) | ✓ |
| "돈을 주웠어" | 💰 재물의 흐름 | ✓ detailFull 307자 |
| "거울 봤는데 무서웠어" | 🪞 자기 직면 | ✓ 301자 |
| "비가 오는 길을 걸었어" | (날씨/길 — 다음 응답) | ✓ 282자 |

확증:
- 본 세션 신규 카테고리 (가족/돈/거울/날씨/길) 정확 매칭
- premium dev_unlock → detailFull 자동 노출 (280~307자 풍부 응답)
- specific 우선순위 정상 (엄마+시험 → 가족 우선)

참고: 테스트 시 응답이 1칸 lag (analyzeDream minLoadTime 2초 + 비동기, 각 입력 3.5초 간격 부족). 실 사용자 단일 입력 = 정상 (앱 동작 무결).

## §12 본 세션 MONGGEUL 마스터 최종 (68 commits)
- dream-demo 34 hardcoded 카테고리 + EXTENDED_DICT 196 + 다중 합성 + 동적 default
- 매칭 키워드 370+
- dali fallback 22 키워드
- premium dev_unlock + canUseDream 무제한
- 보안 XSS 9 라인 esc / sw.js v5 / vite chunk 해소
- 자기 정정 7건
